                    ...
                    else
                    {
                        ...
                        if (nread != 0)
                        {
                            ...
                            case SIGNOUT:
                                ...
                                gameserver_core_status_Ofline(fd);
                                if (send(fd, recv_data, MESSAGE_MAXLEN, 0) != MESSAGE_MAXLEN)
                                {
                            ...
                            case INVITE_SEND:
                                ...
                                sprintf(temp, "%d", fd);
                                if(gameserver_core_checkOnline(enemySide) == 0){
                                    send(fd, message_toString(message_construct(INVITE_DECLINE, "1", "0", "0", "0")), MESSAGE_MAXLEN, 0) ;
                                }else if (send(enemySide, message_toString(message_construct(INVITE_ACCEPT, newMesg->arg, temp, "0", "0")), MESSAGE_MAXLEN, 0) != MESSAGE_MAXLEN)
                                {
                            ...
                            case INVITE_ACCEPT:
                                ...
                                gameserver_core_status_Playing(enemySide);
                                gameserver_core_status_Playing(otherSide);
                                srand(time(NULL));
                                randomNum = rand() % 2;
                            ...
                            case INVITE_DECLINE:
                                ...
                                break;
                            case PLAY_NOW:
                                enemySide = gameserver_core_status_find_Wait();
                                if (enemySide > 0)
                                {
                                    srand(time(NULL));
                                    randomNum = rand() % 2;
                                    ...
                                    sprintf(temp, "%d", randomNum ^ 1);
                                    if (send(fd, message_toString(message_construct(PICK_SIDE, temp, "0", "0", "0")), MESSAGE_MAXLEN, 0) != MESSAGE_MAXLEN)
                                    {
                                        printf("send() sent a different number of bytes than expected\n");
                                    }
                                    gameserver_core_status_Playing(enemySide);
                                    gameserver_core_status_Playing(fd);
                                    sprintf(logStr, "Challege accepted :-< Battle has begun %d vs. %d", enemySide, fd);
                                    logging(logStr);
                                    gameserver_core_initNewRoom(enemySide, fd);
                                }
                                else
                                {
                                    gameserver_core_status_Wait(fd);
                                    if (send(fd, message_toString(message_construct(PLAY_NOW,"0", "0", "0", "0")), MESSAGE_MAXLEN, 0) != MESSAGE_MAXLEN)
                                    {
                                        printf("send() sent a different number of bytes than expected\n");
                                    }
                                    sprintf(logStr, "player is wating %d ", fd);
                                    logging(logStr);
                                }
                                break;
                            case RESULT:
                                printf("%s", newMesg->arg);
                                if (newMesg->arg[0] == '0')
                                {
                                    ...
                                    gameserver_core_status_Online(fd);
                                    gameserver_core_status_Online(enemySide);
                                    gameserver_core_removeRoom(fd);
                                }
                                else if (newMesg->arg[0] == '1')
                                {
                                    gameserver_core_status_Online(fd);
                                    gameserver_core_removeRoom(fd);
                                    if (send(fd, message_toString(message_construct(SIGNOUT, "0", "0", "0", "0")), MESSAGE_MAXLEN, 0) != MESSAGE_MAXLEN)
                            ...
                            case DRAW:
                                ...
                                {
                                    enemySide = gameserver_core_getOtherUserSockfd(fd);
                                    if (send(enemySide, message_toString(message_construct(DRAW, "0", "0", "0", "0")), MESSAGE_MAXLEN, 0) != MESSAGE_MAXLEN)
                                    {
                                        printf("send() sent a different number of bytes than expected\n");
                                    }
                                }
                                else if (newMesg->arg[0] == '0' && newMesg->arg1[0] == '1')
                                {
                                    enemySide = gameserver_core_getOtherUserSockfd(fd);
                                    gameserver_core_status_Online(fd);
                                    gameserver_core_status_Online(enemySide);
                                    gameserver_core_removeRoom(fd);
                                    if (send(enemySide, message_toString(message_construct(DRAW, "1", "0", "0", "0")), MESSAGE_MAXLEN, 0) != MESSAGE_MAXLEN)
                                    {
                                        printf("send() sent a different number of bytes than expected\n");
                                    }
                                    sprintf(logStr, "The war is over :-< ");
                                ...
                                else if (newMesg->arg[0] == '0' && newMesg->arg1[0] == '0')
                                {
                                    enemySide = gameserver_core_getOtherUserSockfd(fd);
                                    if (send(enemySide, message_toString(message_construct(DRAW, "0", "0", "0", "0")), MESSAGE_MAXLEN, 0) != MESSAGE_MAXLEN)
                                    {
                                ...
                                break;
                            case GET_PLAYERLIST:
                                if (send(fd, message_toString(message_construct(GET_PLAYERLIST, gameserver_core_getPlayerlist(fd), "0", "0", "0")), MESSAGE_MAXLEN, 0) != MESSAGE_MAXLEN)
                                {
                                    printf("send() sent a different number of bytes than expected\n");
                                }
